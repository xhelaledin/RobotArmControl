...
namespace Lean.Gui
{
    ...
    public class LeanSnap : MonoBehaviour
    {
        ...
        /// <summary>Snap the RectTransform instantly to (0, -1)</summary>
        public void SnapToZeroMinusOne()
        {
            ...
            float intervalX = horizontalIntervalPixel + horizontalIntervalRect * rect.width + horizontalIntervalParent * parentSize.x;
            float intervalY = verticalIntervalPixel + verticalIntervalRect * rect.width + verticalIntervalParent * parentSize.y;

            Vector2 anchoredPosition = cachedRectTransform.anchoredPosition;

            if (horizontal)
                anchoredPosition.x = position.x * intervalX + horizontalOffset;
            if (vertical)
                anchoredPosition.y = position.y * intervalY + verticalOffset;
